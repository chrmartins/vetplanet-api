...
create table cliente.animal (
    ...
    -- Três estados, cada um com um papel distinto:
    --
    --   ATIVO   aparece na ficha.
    --   OBITO   evento clínico. Tem um dia, a veterinária fica sabendo, e a
    --           tela precisa parar de falar do bichinho como se estivesse vivo.
    --   INATIVO sumiu da lista. É o substituto da exclusão para quando o
    --           animal JÁ TEM histórico e por isso não pode ser apagado.
    --
    -- `INATIVO` NÃO quer dizer "parou de aparecer". Um animal pode passar cinco
    -- anos sem consulta e voltar; basta continuar no cadastro. Ausência é fato
    -- que se observa na data da última consulta, não estado que se declara.
    --
    -- Cadastro criado por engano, sem histórico nenhum, não vira status: é
    -- excluído de verdade (DELETE /api/animais/{id}).
    ...
    constraint animal_situacao_valida
        check (situacao_animal in ('ATIVO', 'INATIVO', 'OBITO')),
...
comment on column cliente.animal.situacao_animal is 'ATIVO, INATIVO (sumiu da lista, tem histórico) ou OBITO. Cadastro errado sem histórico é excluído';
